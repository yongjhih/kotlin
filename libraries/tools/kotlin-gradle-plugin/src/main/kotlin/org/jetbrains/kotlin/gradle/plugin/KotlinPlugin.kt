...
open class KotlinAndroidPlugin @Inject constructor(val scriptHandler: ScriptHandler, val tasksProvider: KotlinTasksProvider) : Plugin<Project> {
    ...
    private fun processVariantData(
        ...
        for (variantData in variantDataList) {
            ...
            val realDestination = kotlinTask.destinationDir
            val versionFile = File(realDestination, "abi_version.txt")
            if (realDestination.exists()) {
                if (!versionFile.exists()) {
                    realDestination.deleteRecursively()
                    kotlinOutputDir.deleteRecursively()
                } else {
                    val abiVersion = JvmBytecodeBinaryVersion(*versionFile.readText().split(".").map { it.toInt() }.toIntArray())
                    if (!abiVersion.isCompatible()) {
                        realDestination.deleteRecursively()
                        kotlinOutputDir.deleteRecursively()
                        versionFile.delete()
                    }
            ...
            if (!versionFile.exists()) {
                realDestination.mkdirs()
                versionFile.createNewFile()
                versionFile.writeText(JvmBytecodeBinaryVersion.INSTANCE.toString())
            }
