...
class OverloadResolver(
    ...
    private fun findRedeclarations(members: Collection<CallableMemberDescriptor>): Set<Pair<KtDeclaration, CallableMemberDescriptor>> {
        ...
        for (member in members) {
            for (member2 in members) {
                ...
                if (!OverloadUtil.isOverloadable(member, member2) && member.kind != CallableMemberDescriptor.Kind.SYNTHESIZED) {
                    if (isTopLevelMainInDifferentFiles(member, member2)) {
                        continue
                    }

                    val ktDeclaration = DescriptorToSourceUtils.descriptorToDeclaration(member) as KtDeclaration?
                    redeclarations.add(Pair.create<KtDeclaration, CallableMemberDescriptor>(ktDeclaration, member))
                }
            }
        }
    ...
    private fun reportRedeclarations(redeclarations: Set<Pair<KtDeclaration, CallableMemberDescriptor>>) {
        ...
        for (redeclaration in redeclarations) {
            val ktDeclaration = redeclaration.getFirst() ?: continue
            val memberDescriptor = redeclaration.getSecond()
