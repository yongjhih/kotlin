...
class NativeInnerClassChecker : DeclarationChecker {
    override fun check(declaration: KtDeclaration, descriptor: DeclarationDescriptor, diagnosticHolder: DiagnosticSink,
        ...
        if (descriptor.isInner && !AnnotationsUtils.isNativeObject(getOuterClass(descriptor))) {
            diagnosticHolder.report(ErrorsJs.NATIVE_INNER_CLASS_PROHIBITED.on(declaration))
        }
    }

    private fun getOuterClass(descriptor: DeclarationDescriptor): ClassDescriptor {
        var desc = descriptor
        do {
            desc = desc.containingDeclaration!!
        } while (desc !is ClassDescriptor)
        return desc
    }
}
